...
class MultipeerCommunicator: NSObject, Communicator, MCSessionDelegate, MCNearbyServiceBrowserDelegate, MCNearbyServiceAdvertiserDelegate {
    ...
    override init() {
        ...
        advertiser = MCNearbyServiceAdvertiser(peer: peer, discoveryInfo: ["userName": UIDevice.current.name], serviceType: serviceType)
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        do {
            let jsonMessage = try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as! Dictionary<String, String>
